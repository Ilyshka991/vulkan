...
#if defined(VKB_DEBUG) || defined(VKB_VALIDATION_LAYERS)
    VK_CHECK(vkCreateDebugReportCallbackEXT(context.instance, &debug_report_create_info, nullptr,
...
void HelloTriangle::init_device(Context &context,
    ...
    VK_CHECK(vkEnumerateDeviceExtensionProperties(context.gpu, nullptr, &device_extension_count,
...
VkResult HelloTriangle::acquire_next_image(Context &context, uint32_t *image) {
    ...
    VkResult res = vkAcquireNextImageKHR(context.device, context.swapchain, UINT64_MAX, acquire_semaphore,
